...
```{r fig.height=3,fig.width=10,fig.cap="This plot shows the changes in flexion scores for each of the patients, showing the left and right hip next to one another. Note that the observations to the left of the black line are from the control group and to the right are the treatment group. Observations with very little change between the before and after periods are shown with sideways arrowheads."}
# data %>%
...
```{r fig.height=3,fig.width=10,fig.cap="This plot shows the changes in rotation scores for each of the patients, showing the left and right hip next to one another. Note that the observations to the left of the black line are from the control group and to the right are the treatment group. Observations with very little change between the before and after periods are shown with sideways arrowheads."}
# Figure 2
# data %>%
...
We will build a multi mixed-effect linear model for this project as we are interested in modelling both random and fixed effects. The response variables will be *flexionAfter* and *rotationAfter* for both abilities\footnote{At first, it may seem more logical to use the difference between the before and after scores as a response and not include the score before as a predictor. We chose the method that we did, however, due to the potential differences in the amount of change based on where the patient started. Considering only changes from before to after scores excludes the possibility that patients with a lower starting score may have different tendencies to improve than those starting with a higher score.}. The independent variables include *flexionBefore* and *rotationBefore* as random effects variables, *hip* (right or left) and *treatment* (control or stretch) are fixed effects variables. *patientNo* will also be random effects variable since for each patient there is right hip data and left hip data. 
...
```{r fig.height=3,fig.weight=10,fig.cap="These plots show the residuals for both of our models. Notice that the residuals are distributed around 0 with no distinct pattern."}
par(mfrow=c(2,2))
gridExtra::grid.arrange(
...
```{r fig.height=3,fig.weight=10,fig.cap="These plots test the normality assumption for both of our models. Notice that in both cases, the points fall very close to the line."}
gridExtra::grid.arrange(
...
## B: Scatter plot for flexionIncrease and RotationIncrease 
...
data %>%
  ...
  geom_abline(slope = 1)+
  ggtitle("Comparing Rotation Before and After Treatment")
```
...
## C: Correlation plot for flexionIncrease and RotationIncrease on both hips
...
data %>%
  ...
  geom_vline(xintercept = 0) + 
  geom_hline(yintercept = 0) +
  ggtitle("Rotation Increase")
,nrow = 1,
bottom = grid::textGrob("Figure 9: These plots compare the increases of left and right hips of the same patient.",
                           x = 0,
                           y = 0.5,
                           just = "left"))
```
