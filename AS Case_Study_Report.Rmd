...
$$flexionAfter_{ij} = \beta_1*flexionBefore_{ij} + \beta_2*hip + \beta_3*treatment + \beta_{4_i} + \epsilon_{ij}$$

$$\beta_{4_i} = \alpha_i +  \epsilon_i$$
where hip = 0 if it is a left hip and 1 if it is a right hip. Treatment = 0 if it is the control and 1 if it is the treatment group, i represents the patient number and j represents which hip is being considered
...
```
```{r}
summary(model1Flexion)
summary(model1Rotation)
```
