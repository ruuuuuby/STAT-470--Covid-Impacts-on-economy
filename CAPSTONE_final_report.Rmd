...
After we remove the linear trend of the model, we check the residuals and decide to apply AR(1), Autoregressive Model (1). It means that current values will depend on previoud one time values. The residuals after AR model with removed linear trend is *Figure 8*. 

*Figure 8* shows the acf of residuals plot. Seems for all lags, the correlation is insignificant, we can conclude that the residuals are independent. The residuals should follow the assumptions of normality, equal variance, and independence. More assumptions checks can be found in *Appendix D*. In conclusion, this model approach is valid. 
...
```{r, fig.cap = "residuals of models, showing the real effects of shocks on percent change", fig.height=3}
# linear model 
...
Similar to approach 1 for mothly model, we begin with removing the trend and apply AR(1) model. The results correspond to our discoveries before and the residuals correspond to assumptions so we conclude that the approach is valid and can apply cross correlation approach to the model. 
...
# Appendix E: Subsetting model results
