...
We also consider a daily data set, i.e. data reported for each day, which consists of the DOW Jones Composite, gold price, S&P 500, New York Stock Exchange, and 10 year bond rates. A more detailed explanation of these variables is in *Table 1* and *Table 2*. 
...
#DowDiff<-diff(log(data$DOW_Close),1)
...
Of course, we also want to consider the explanatory variables and their relationship to the DOW. Further plots and analysis considering these variables can be found in Appendix A.
...
# Difference between residuals
#res_tempd<-diff(regd$residuals, 1)
...
For the monthly data, we choose the response variable as *Dow_Close* value. We also choose 6 monthly reported economic predictors to measure the structure. Our goal is to examine how the stock market (the economy, as measured by the DOW Jones Composite Average) will respond to changes in macro-variables. To normalize the data, we take log of the difference of all our variables except for inflation, which is equal to take the percent change of our values. More importantly, such transformation can satisfy our assumption of stationarity on response variable. 
...
After we remove the linear trend of the model, we check the residuals and decide to apply AR(1), Autoregressive Model (1).
AR(1) is a time series model where we assume that each current observation depends on the value directly preceding it. The residuals after AR model with removed linear trend is *Figure 8*. 

*Figure 8* shows the acf of residuals plot. Seems for all lags, the correlation is insignificant, we can conclude that the residuals are independent. The residuals should follow the assumptions of normality, equal variance, and independence. More assumptions checks can be found in *Appendix B*. In conclusion, this model approach is valid. 

*Figure 9* shows the residuals with red bubbles shocks. It corresponds to our identification before. However, an issue here is that when we remove linear trend before, we remove the trend of current *Dow_Close* and current predictors. It is common that current *Dow_Close* will not only depend on current predicotors, but also some previous predictors. Thus, we need to have a further approach to take all lags that are significant into consideration. 
...
```{r, fig.cap = "This plot shows the residuals of models, showing the real effects of the shocks on percent change. The points in red correspond to the points in the months identified as shock periods by our monthly shock identification process.", fig.height=3}
# linear model 
...
Similar to approach 1 for monthly model, we begin with removing the trend and apply AR(1) model. The results correspond to our discoveries before and the residuals correspond to assumptions so we conclude that the approach is valid and can apply cross correlation approach to the model. 
...
```{r,fig.cap = "This figure shows cross correlation between Dow and 4 daily predictors.The x-axis is the lag, while before 0 represents past and after future. Future analysis is meaningless here since we will not do prediction. Lag that is higher than blue line means it is significant"}
par(mfrow =c(2,2))
...
Using the daily data to analyze on specific shocks we have identified before, we noticed that the shocks would usually last over a month. The 1987 and 1990 shocks seemed to last for one month, while other shocks, specifically between 1998 and 2001 had some longstanding effects that the economy were declining during 1998 and 2001 since most changes were negative. Also, a similar pattern showed up after 2001. However, the current month Dow changes were still pretty stable. One explanation is that even if there is a shock, people need more than one days to react the shocks. 

Also, we checked the pattern of 2009 Great recession and observe that it has the most variation. Thus, during this period, people seemed to face stronger shocks. A similar pattern showed up for March coronavirus shock that it has largest range among the shocks. It can be effects of both financialling stimulation by government and pandemic. 

```{r,fig.cap="This plot shows the residuals of the model, showing the real effects of shocks on percent change. The red points represent the points from the months that were identified as shock periods in our monthly shock identification step.", fig.height=3}
# linear model 
...
points(ymd(datad$DATE)[1713:1734],resid(modelccfd)[1695:1716],col="red")
points(ymd(datad$DATE)[2429:2451],resid(modelccfd)[2411:2433],col="red")
points(ymd(datad$DATE)[4452:4472],resid(modelccfd)[4434:4454],col="red")
points(ymd(datad$DATE)[5231:5245],resid(modelccfd)[5213:5227],col="red")
points(ymd(datad$DATE)[7009:7031],resid(modelccfd)[6991:7013],col="red")
points(ymd(datad$DATE)[7093:7111],resid(modelccfd)[7075:7093],col="red")
points(ymd(datad$DATE)[9881:9902],resid(modelccfd)[9863:9884],col="red")
```
...
In order to test if the magnitudes of the residuals for shock periods are larger than for non-shock periods, we used permutation testing, the methods and results for which are given in much more detail in Appendix C. The results showed that the monthly analysis shows very strong evidence that the shocks have higher magnitude residuals in both models. The daily analysis also showed evidence that the shock periods, both those identified using the methods for daily analysis and for those identified using monthly analysis, have higher magnitude residuals, though not quite as strong of evidence compared to the monthly.

# Discussion

For the study, we use monthly and daily *DOW_Close* value to be our response variables. By the residuals of *DOW_Close* with time, we recognize 7 shocks in total. To analyze the real effects of these shocks, we run a model on both monthly and daily data using the assumptions of economic structure. 

The result of monthly data shows that all shocks create 10% decrease in monthly *DOW_Close* values. Thus, we assume natural fluctuations in a month occur in a range of 10%. Among the shocks, we recognize that almost all shocks created 10% decrease except for the Black Monday Stock Market Shock in October, 1987. The reason we assume is that this shock has direct impact on stock market. On the other hand, other shocks are stable on 10% decrease because economy and government have the ability to adjust the market. On the other hand, from the daily data, we notice that nearly all shocks have effects over one month. The typical indicator of a shock is a percent change rate of 2% decrease. Shocks period didn't show many differences compared to non-shock periods since people need time to react after shocks. So the differences were significant usually after some days of the shocks. Also, compared to different shocks, we observe that shocks between 1998 and 2001 had long-lasting effects, while covid shock has large fluctuations. 

To bring this back to our research questions, our analysis suggests that shock periods do generally follow a different economic structure than other periods in the sense that they are not fit as well by our models as other points. In this sense, the shock points appeared to be similar in that they generally did not match the general trend in either of our models. However, the differences appeared in the magnitudes of these differences. For example, the COVID-19 shock often, both in the models and in the plots of the individual predictors, showed up as a more extreme shock than others. This is similar to the Black Monday shock in its severity, but keep in mind that Black Monday was centered around a very short time period compared to COVID's longer term effect. In this sense, COVID-19 shows similarity to the shocks in the Great Recession in that while only an individual shock point was detected, yet there are lingering effects nearby the identified shock, just not extreme enough to be considered a shock.



# Limitations and Future Study

One major limitation to our analysis is the set of implicit assumptions that we made. We considered the DOW as a metric for the economy, but this is not the only possible metric for economic performance. For example, we could have used real GDP, a variable excluded from this analysis due to its quarterly nature, as a metric nearly as easily as the DOW. Generalizing this point, we assumed that the relationships between our selected variables could properly capture the structure of the economy, which is introducing another assumption that may not fully accurately represent reality. As such, we would suggest that future studies consider not only other prominent metrics for economic structure but also other modelling structures in order to provide stronger evidence for the nature of the economic structure of shock periods.

We also made assumptions on what is considered a shock. Our shock detection method identified only shocks with a rapid effect in a particular month, which excludes cases like the implementation of the Internet that was a shock to the economy but happened slightly more gradually. The identification was also entirely dependent on our data, so introducing other shock identification methods would be a good opportunity for further study.

Statistically, we note that the variables for DOW, S&P 500, and nyse are very similar, even sharing components between them. While our analysis predominantly ignores the issues, particularly in regards to collinearity, that this contributes to, we do acknowledge that these issues do hinder our models. Even though the constructed models themselves are not the true purpose of the analysis and our analysis was limited by the data we had, we would suggest further analysis including models with more constraints applied to limit collinearity, possibly including some other daily variables that are less similar to one another.

We also acknowledge that our models explained very little of the variation. However, this is to be expected as economic theory suggests that the changes in the DOW cannot be well predicted. In fact, our original goal was not to model the response variable but rather to use the model as a metric for economic structure.
...
\newpage
# Appendix A: Additional Plots and Exploratory Analysis
...
\newpage
# Appendix B: Assumption checks for modelling economic structure

For the modelling part, when we subset the predictors, we use linear model. Thus, we need to satisfy the assumptions of linear model that residuals should be normally, independently, identically distributed. 

```{r app7, fig.cap="Residuals plot for monthly model"}
par(mfrow=c(2,2))
plot(modelccf,which=1:4)
acf(resid(modelccf))
pacf(resid(modelccf))
```
```{r app8, fig.cap="Residuals plot for daily model"}
par(mfrow=c(2,2))
plot(modelccfd,which=1:4)
acf(resid(modelccfd))
pacf(resid(modelccfd))
```



\newpage
# Appendix C: Full Permutation Test Descriptions
...
hist(T_perm, xlim = c(0,.16), main = "Samples for Monthly Model 1")
abline(v=T0, col = "red")
...
hist(T_perm, xlim = c(0,.16), main = "Samples for Monthly Model 2")
abline(v=T0, col = "red")
p2 <- mean(abs(T_perm)>=abs(T0))
```

```{r permtest3, fig.cap="This figure shows the sampling distribution generated for daily model 1 through permutation testing. The red line shows the observed value. The value for 557 of the 10000 samples is less than the observed value, i.e. a p value of .0557, providing support that the shock residuals have greater magnitude than the other residuals.", fig.height=3}
...
hist(T_perm, main = "Samples for Daily Model 1\nUsing Daily-Generated Shocks")
abline(v=T0, col = "red")
...
```{r permtest32, fig.cap="This figure shows the sampling distribution generated for daily model 1 through permutation testing. The red line shows the observed value. The value for 1100 of the 10000 samples is less than the observed value, i.e. a p value of .11, providing support that the shock residuals, using the same shocks as in the monthly data, have greater magnitude than the other residuals.", fig.height=3}
# Permutation testing model 1 daily
...
hist(T_perm, main = "Samples for Daily Model 1\nUsing Monthly-Generated Shocks")
abline(v=T0, col = "red")
p4 <- mean(abs(T_perm)>=abs(T0))
```


```{r permtest4, fig.cap="This figure shows the sampling distribution generated for daily model 2 through permutation testing. The red line shows the observed value. The value for 469 of the 10000 samples is less than the observed value, i.e. a p value of .0469, providing support that the shock residuals have greater magnitude than the other residuals.", fig.height=3}
# Permutation testing model 2 daily with daily shocks
...
hist(T_perm, main = "Samples for Daily Model 2\nUsing Daily-Generated Shocks")
abline(v=T0, col = "red")
p5 <- mean(abs(T_perm)>=abs(T0))
```


```{r permtest5, fig.cap="This figure shows the sampling distribution generated for daily model 2 through permutation testing. The red line shows the observed value. The value for 26 of the 10000 samples is less than the observed value, i.e. a p value of .0026, providing support that the shock residuals, using the same shocks as in the monthly data, have greater magnitude than the other residuals.", fig.height=3}
# Permutation testing model 1 daily with monthly shocks
outd <- c(1694:1715, 2410:2432, 4433:4453, 5212:5226, 6990:7012, 7074:7092, 9862:9883)
...
# Appendix D: Subsetting model results

```{r}
out<-broom::tidy(modelccf)
knitr::kable(out, caption = "Monthly data subsetted model results")
```
```{r}
out<-broom::tidy(modelccfd)
knitr::kable(out, caption = "Daily data subsetted model results")
```
