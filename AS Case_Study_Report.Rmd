...
The descriptive statistics will show the summary statistics including min, max, median, mean, and variance for both control and treatment groups. For the descriptive statistics, we will not separate the left and right hips, rather choosing to explore the differences between left and right hips graphically. Also note that we have added two variables, *flexiondiff* and *rotationdiff*, which are the changes in flexion and rotation respectively from the before score to the after score.
